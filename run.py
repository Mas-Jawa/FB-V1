...
try :
    ...
    console.print(f" {K2}• {H2}Sedang Menginstall Modul Pillow {H2}•{P2}")
...
# ------------------[ PROXY BUAT NONTON BKP BTW FENDI GAMTENG ]-------------------#
try:
    prox = requests.get(
...
try:
    ...
    ugent = "Mozilla/5.0 (Linux; Win64; Xiaomi Redmi Note 9 Build/NRD90M; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Gecko/20100101 Firefox/99"
...
def banner():
    Console().print(
        Panel(
            """
[bold red]███████████████████████ [bold yellow]NAME  : [bold green]Fendi Gamteng 
...
[{color_text}05{P2}]. {M2}EXIT{P2}""",width=60,title="MENU",style=f"{color_panel}"))
    ...
    if HaHi in ["1", "01"]:
    	...
    	console.print(f" {H2}• {P2}[bold red]Masukan Yang Bener Tolol!!! btw Fendi gamteng ")
...
def metslow():
    ...
    Console().print(
        Panel(
            f"[bold green]Crack Telah Selesai ngap, btw Fendi Gamteng",
            subtitle="╭───",
            subtitle_align="left",
...
def fankygraphv1(idf, pwv, url):
    ...
    #ua = f"Mozilla/5.0 (Linux; Android 10.0; (Xiaomi Redmi Note 9 Build/NRD90M; Win64; x64; rv:99.0) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/121.0.6167.143 Gecko/20100101 Firefox/99.0 Trailer/92.3.2312.13"
...
def fankywww(idf, pwv):
    ...
    # ua = "Mozilla/5.0 (Linux; Android 10.0; (Xiaomi Redmi Note 9 Build/NRD90M; Win64; x64; rv:99.0) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/121.0.6167.143 Gecko/20100101 Firefox/99.0 Trailer/92.3.2312.13"
    ua = random.choice(ugen)
    for pw in pwv:
        try:
            if 'ya' in ualuh: 
